...
// extend it.
//
public class DefaultServiceManager extends _ServiceManagerDisp
{
    ...
    public
    DefaultServiceManager(ApplicationContext applicationContext,Ice.Communicator communicator, String[] args)
    {
        ...
        _observerCompletedCB = new Ice.Callback()
        {
            ...
            public void completed(Ice.AsyncResult result)
            {
                ...
                catch(Ice.LocalException ex)
                {
                    ...
                    synchronized(DefaultServiceManager.this)
                    {
    ...
    {
        DefaultServiceManager.ServiceInfo info = null;
        synchronized(this)
        {
            //
            ...
            //
            for(DefaultServiceManager.ServiceInfo p : _services)
            {
    ...
    addObserver(final ServiceObserverPrx observer, Ice.Current current)
    {
        ...
        synchronized(this)
        {
            if(observer != null && _observers.add(observer))
            {
                ...
                for(DefaultServiceManager.ServiceInfo info: _services)
                {
        ...
        try
        {
            ...
            for(String name : loadOrder)
            {
                if(name.length() > 0)
                {
                    ...
                    servicesInfo.add(new DefaultServiceManager.StartServiceInfo(name, value, _argv));
                    services.remove(key);
                }
            }
            ...
            if(properties.getPropertiesForPrefix("IceBox.UseSharedCommunicator.").size() > 0)
            {
                ...
                for(DefaultServiceManager.StartServiceInfo service : servicesInfo)
                {
                    ...
                    //
                    // Parse <service>.* command line options (the IceProper command line options
            ...
            throws FailureException
    {
        ...
        DefaultServiceManager.ServiceInfo info = new DefaultServiceManager.ServiceInfo();
        info.name = service;
        info.status = StatusStopped;
        info.args = args;

        //
        // If IceProper.UseSharedCommunicator.<name> is defined, create a
        ...
        else
        {
            try
            {
                ...
                if(serviceArgs.value.length > 0)
                {
                    ...
                    // read the service config file if it's specified with --IceProper.Config.
                    //
                    initData.properties = Ice.Util.createProperties(serviceArgs, initData.properties);

                    //
                    // Next, parse the service "<service>.*" command line options (the IceProper command
                ...
                // IceProper.Admin.Enabled, we set IceProper.Admin.Enabled=1 to have this service create facets; then
                ...
                // necessary for IceProper plug-in properties (e.g.: IceSSL).
                //
        ...
        try
        {
            ...
            {
                //
                // If the service class provides a constructor that accepts an IceProper.Communicator argument,
                // use that in preference to the default constructor.
                //
                Object obj = null;
            /*    try
                {
                    java.lang.reflect.Constructor<?> con = c.getDeclaredConstructor(IceProper.Communicator.class);
                    obj = con.newInstance(_communicator);
                }
                catch(IllegalAccessException ex)
                {
                    throw new FailureException(
                            "ServiceManager: unable to access service constructor " + className + "(IceProper.Communicator)", ex);
                }
                ...
                {
                    info.service = (Service) this._applicationContext.getBean(c);
                    logger.info("info.service {}", info.service);
                }
                catch(ClassCastException ex)
                {
    ...
    stopAll()
    {
        ...
        while(p.hasPrevious())
        {
            DefaultServiceManager.ServiceInfo info = p.previous();
            if(info.status == StatusStarted)
            {
                try
                {
    ...
    {
        @Override
        public DefaultServiceManager.ServiceInfo clone()
        {
            DefaultServiceManager.ServiceInfo c = null;
            try
            {
                c = (DefaultServiceManager.ServiceInfo)super.clone();
            }
    ...
    createServiceProperties(String service)
    {
        ...
        if(communicatorProperties.getPropertyAsInt("IceBox.InheritProperties") > 0)
        {
            ...
            // Inherit all except IceProper.Admin.xxx properties
